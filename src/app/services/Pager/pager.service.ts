  ...
  implements PagerInterface, PagerNavigationInterface, PagerTemplateInterface
{
  ...
  #pagerIndexList: Array<number> = [];
  #pagerItemList: PagerDisplayedItemsInterface = {} as PagerDisplayedItemsInterface;
  public currentPage: Signal<number> = toSignal(
    this.#currentPage.pipe(filter((value) => !!value)),
    {
      initialValue: 1,
    },
  );
  public currentOffset = computed(() =>
    this.getOffsetForPage(this.currentPage()),
  );
